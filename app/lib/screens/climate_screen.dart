...
class _ClimateScreenState extends State<ClimateScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      double lat = 10.7867, lng = 79.1378;
      String place = '';
      ...
      // Geocode
      place = await SatelliteService.geocode(lat, lng);

      final forecast = await WeatherService().getForecast(lat: lat, lng: lng);
      if (!mounted) return;
      setState(() {
        _forecast = forecast;
        _location = place.isNotEmpty ? place : '$lat, $lng';
        _loading = false;
      });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    if (_loading) {
      return Scaffold(
        appBar: AppBar(title: Text(AppStrings.of(context).climateRiskTitle)),
    ...
    if (_error != null) {
      return Scaffold(
        ...
        body: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
          const Icon(Icons.cloud_off_rounded, size: 48, color: kTextGrey),
          const SizedBox(height: 12),
          Text('Could not load weather data', style: const TextStyle(color: kTextGrey)),
          const SizedBox(height: 12),
    ...
    final risk = WeatherService.summariseRisk(_forecast);
    final drought = (risk['drought']! * 100);
    final flood   = (risk['flood']!   * 100);
    final heat    = (risk['heat']!    * 100);
    final overall = (risk['overall']! * 100);
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Container(
              ...
              child: Row(children: [
                ...
                Text('${_forecast.length}-Day Forecast',
            ...
            const SizedBox(height: 16),
            _ForecastTable(forecast: _forecast),
            const SizedBox(height: 16),
            if (heat > 35) _AlertBox(heat: heat),
            const SizedBox(height: 16),
            Row(children: [
...
class _AlertBox extends StatelessWidget {
  final double heat;
  const _AlertBox({required this.heat});
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ...[AppStrings.of(context).heatAlertTip1,
              AppStrings.of(context).heatAlertTip2,
              AppStrings.of(context).heatAlertTip3]
              .map((tip) => Padding(
